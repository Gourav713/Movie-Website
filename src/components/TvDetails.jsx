...
function TvDetails() {
  ...
  return info ? (
    <div
      ...
      className="relative w-screen min-h-screen px-[6%] py-4"
    >
      {/* Navigation */}
      <nav className="h-[10vh] w-full text-zinc-100 flex items-center gap-6 text-xl">
        ...
        <a
          ...
          title="IMDb"
        >
          <img src={imdb} alt="IMDb" className="w-10 h-6 object-contain" />
        </a>
      </nav>
      ...
      <h1 className="text-3xl font-semibold text-white mb-4">Seasons </h1>
      <div className="w-full overflow-x-auto flex gap-5 mt-5 mb-5 scrollbar-hide">
        {info.detail.seasons.map((item, index) => (
          <div
            key={item.id || index}
            className="flex-shrink-0 flex flex-col items-center w-[160px] min-w-[140px] max-w-[180px]"
          >
            <img
              className="h-[24vh] w-full shadow-[8px_17px_38px_2px_rgba(0,0,0,.5)] object-cover rounded-lg"
              src={
            ...
            />
            <h1 className="text-base text-zinc-300 mt-2 font-semibold text-center w-full truncate">
              {item.name ||
                item.title ||
                item.original_name ||
                item.original_title}
