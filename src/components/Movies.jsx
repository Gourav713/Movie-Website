...
function Movies() {
  document.title = "MoviePlex | Movies";
  ...
  const getMovie = async (resetData = false) => {
    if (loading) return;
    ...
    try {
      ...
      setHasMore(currentPage < data.total_pages && data.total_pages <= 500); // TMDb limit
    } catch (error) {
  ...
  const handleSortChange = (newSort) => {
    setSortBy(newSort);
    // Note: You might need to implement custom sorting logic here
    // as TMDb API doesn't support all sort options for all endpoints
  };
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  <i className="ri-movie-2-fill text-blue-500 text-2xl"></i>
            ...
            <div className="text-center py-8">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-gray-800 rounded-lg">
                ...
                <span className="text-gray-300">
                  You've seen all {movie.length} movies!
                </span>
              </div>
            </div>
          }
        >
          <Cards data={movie} title="movie" viewMode={viewMode} />
        </InfiniteScroll>
          ...
          <div className="mt-8 text-center">
            <div className="inline-flex items-center gap-4 px-6 py-3 bg-gray-800/50 rounded-full">
              <div className="flex items-center gap-2 text-sm text-gray-400">
                ...
                <span>
                  Showing {movie.length} of {totalResults.toLocaleString()}
                </span>
              </div>
